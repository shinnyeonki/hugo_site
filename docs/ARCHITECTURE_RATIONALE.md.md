...
이 문서는 **왜 이런 복잡한 아키텍처를 선택했는가**에 대한 근본적인 이유와 철학을 설명합니다.
...
나는 평소 공부나 여러 가지를 기록하는 습관이 있다. 그러나 노트 플랫폼을 이동한 경험이 있다:
...
5. **미래 보장**: 단수 text 파일이므로 순수하게 읽을 수 있음
...
1. index.html 로드
  ↓
2. 사전 생성된 JSON 파일 로드 ( 파일 인덱스, 검색 인덱스, tag 인덱스, graph 인덱스 )
  ↓
3. 좌측 네비게이션 렌더링 (파일별 트리, 검색 트리, tag 트리, recent 트리)
  ↓
4. 특정 파일을 클릭하면 Markdown 파일 비동기 fetch
  ↓
...
최근 google 검색엔진이 javascript 렌더링을 어느 정도 지원한다고 하지만, 여전히 한계가 많다.
path 기반으로 특정 페이지임을 나타내야 하지만 서버측 웹서버 설정 변경없이는 해결이 힘들다 이것 또한 404.html 으로 처리한다고는 하지만 이러한 것들이 검색엔진의 인덱스 생성에 큰 장애물이 된다.
